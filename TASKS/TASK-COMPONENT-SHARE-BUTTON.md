# COMPONENT-SHARE-BUTTON — URL 공유 버튼
...
## Context
URL 공유 버튼. 이 Task는 하나 이상의 Page Owner Task가 의존하는 화면 구성요소를 구현한다. 대상 범위: SCR-001, SCR-002, SCR-004. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 10에 대응한다.
...
## Functional AC
Web Share API 우선, 실패 시 클립보드 복사 폴백
...
## Test Cases
- Functional AC(`Web Share API 우선, 실패 시 클립보드 복사 폴백`)를 충족하는 동작 확인
- 검증 경로: MANUAL-CHECK-EXTERNAL-LINKS
...
- [ ] Verify(MANUAL-CHECK-EXTERNAL-LINKS) 통과
