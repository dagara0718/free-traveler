...
## Context
신고 생성/관리자 상태변경. 이 Task는 Supabase 기반 서버 로직(Server Action/Route Handler)을 구현한다. 대상 범위: SCR-004, SCR-005. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 38에 대응한다.

## Project Scope
Implementation Status: **IMPLEMENT** (`docs/PROJECT_SCOPE.md`, `docs/UIUX_TRACEABILITY.md` 기준).

## Requirement Ref
REQ-FUNC-039,REQ-FUNC-041,REQ-FUNC-044; REQ-NF-019
...
## Expected Files
`src/app/api/reports/route.ts`, `src/app/api/admin/reports/[id]/route.ts`

이 목록 밖의 파일은 이 Task에서 수정하지 않는다.

## Functional AC
신고 3초 이내 접수; 상태 필터 조회

## Visual AC
해당 없음

## Security/Privacy AC
신고 상세 Moderator/Admin만 열람(RLS)

## Test Cases
- Functional AC(`신고 3초 이내 접수; 상태 필터 조회`)를 충족하는 동작 확인
