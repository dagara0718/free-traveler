# COMPONENT-SC002-PICKS-CTA — 추천 여행지 4 + CTA 배너
...
## Context
추천 여행지 4 + CTA 배너. 이 Task는 하나 이상의 Page Owner Task가 의존하는 화면 구성요소를 구현한다. 대상 범위: SCR-002. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 20에 대응한다.
...
## Expected Files
`src/components/about/TopPicks.tsx`

이 목록 밖의 파일은 이 Task에서 수정하지 않는다.

## Functional AC
추천 4개, 비공개 여행지 자동 제외; CTA 2개(`/travel-tools`,`/mates`)
...
## Test Cases
- Functional AC(`추천 4개, 비공개 여행지 자동 제외; CTA 2개(`/travel-tools`,`/mates`)`)를 충족하는 동작 확인
