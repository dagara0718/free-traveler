...
## Context
동행글 목록/상세 조회. 이 Task는 Supabase 기반 서버 로직(Server Action/Route Handler)을 구현한다. 대상 범위: SCR-001, SCR-004. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 34에 대응한다.

## Project Scope
Implementation Status: **IMPLEMENT** (`docs/PROJECT_SCOPE.md`, `docs/UIUX_TRACEABILITY.md` 기준).

## Requirement Ref
REQ-FUNC-030,REQ-FUNC-033,REQ-FUNC-037,REQ-FUNC-044
...
## Expected Files
`src/lib/server/mates.ts`

이 목록 밖의 파일은 이 Task에서 수정하지 않는다.

## Functional AC
필터+겹침기간+차단제외 쿼리; p95 목표 1초(정식 부하측정 제외)
...
## Test Cases
- Functional AC(`필터+겹침기간+차단제외 쿼리; p95 목표 1초(정식 부하측정 제외)`)를 충족하는 동작 확인
