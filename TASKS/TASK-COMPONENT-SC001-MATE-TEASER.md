# COMPONENT-SC001-MATE-TEASER — 최근 동행글 3개/Empty State
...
## Context
최근 동행글 3개/Empty State. 이 Task는 하나 이상의 Page Owner Task가 의존하는 화면 구성요소를 구현한다. 대상 범위: SCR-001. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 14에 대응한다.
...
## Expected Files
`src/components/home/MateTeaser.tsx`

이 목록 밖의 파일은 이 Task에서 수정하지 않는다.

## Functional AC
모집중 글 최대 3개 또는 완성형 Empty State
...
## Test Cases
- Functional AC(`모집중 글 최대 3개 또는 완성형 Empty State`)를 충족하는 동작 확인
- 검증 경로: 수동 QA
