# COMPONENT-SC005-ADMIN — Admin 관리자 Tab
...
## Context
Admin 관리자 Tab. 이 Task는 하나 이상의 Page Owner Task가 의존하는 화면 구성요소를 구현한다. 대상 범위: SCR-005. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 33에 대응한다.
...
## Functional AC
신고 상태 필터(OPEN/REVIEWING/RESOLVED/DISMISSED)+카드; 항공·숙소 외부 URL HTTPS 저장

## Visual AC
통계 대시보드/차트 없음

## Security/Privacy AC
HTTP·javascript: URL 저장 거부

## Test Cases
- Functional AC(`신고 상태 필터(OPEN/REVIEWING/RESOLVED/DISMISSED)+카드; 항공·숙소 외부 URL HTTPS 저장`)를 충족하는 동작 확인
- 검증 경로: 수동 QA
