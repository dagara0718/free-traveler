# COMPONENT-SC003-FLIGHT-FORM — 항공 조건 Form+요약+외부이동
...
## Context
항공 조건 Form+요약+외부이동. 이 Task는 하나 이상의 Page Owner Task가 의존하는 화면 구성요소를 구현한다. 대상 범위: SCR-003. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 22에 대응한다.
...
## Expected Files
`src/components/travel-tools/FlightForm.tsx`, `src/components/travel-tools/SummaryCard.tsx`

이 목록 밖의 파일은 이 Task에서 수정하지 않는다.

## Functional AC
국가·지역·출발일·귀국일; 과거/역전 날짜 차단; 요약 후 새 탭 외부이동(`noopener,noreferrer`)

## Visual AC
비전달 고지 문구 상시 노출

## Security/Privacy AC
입력값 서버·DB·URL query 미전송(클라이언트 상태 한정)

## Test Cases
- Functional AC(`국가·지역·출발일·귀국일; 과거/역전 날짜 차단; 요약 후 새 탭 외부이동(`noopener,noreferrer`)`)를 충족하는 동작 확인
