...
## Context
동행글 생성/수정/마감. 이 Task는 Supabase 기반 서버 로직(Server Action/Route Handler)을 구현한다. 대상 범위: SCR-003, SCR-005. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 35에 대응한다.

## Project Scope
Implementation Status: **IMPLEMENT** (`docs/PROJECT_SCOPE.md`, `docs/UIUX_TRACEABILITY.md` 기준).

## Requirement Ref
REQ-FUNC-031,REQ-FUNC-035,REQ-FUNC-038,REQ-FUNC-044; REQ-NF-021
...
## Expected Files
`src/app/api/mates/route.ts`, `src/lib/server/mates.ts`

이 목록 밖의 파일은 이 Task에서 수정하지 않는다.

## Functional AC
필수값/날짜검증 서버측 재검증; 중복 활성 요청 unique 제약 연동

## Visual AC
해당 없음

## Security/Privacy AC
동일 사용자 과다 생성 간단 rate limit

## Test Cases
- Functional AC(`필수값/날짜검증 서버측 재검증; 중복 활성 요청 unique 제약 연동`)를 충족하는 동작 확인
- 검증 경로: UNIT-MATE-STATE
