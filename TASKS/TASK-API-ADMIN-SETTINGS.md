...
## Context
외부 URL 설정 저장. 이 Task는 Supabase 기반 서버 로직(Server Action/Route Handler)을 구현한다. 대상 범위: SCR-005. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 39에 대응한다.
...
## Expected Files
`src/app/api/admin/settings/outbound/route.ts`
...
## Security/Privacy AC
HTTP/`javascript:`/`data:` 거부

## Test Cases
- Functional AC(`HTTPS+허용목록만 저장`)를 충족하는 동작 확인
- 검증 경로: 수동 QA
