# COMPONENT-SC002-TIMELINE — 여행 Timeline
...
## Context
여행 Timeline. 이 Task는 하나 이상의 Page Owner Task가 의존하는 화면 구성요소를 구현한다. 대상 범위: SCR-002. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 17에 대응한다.
...
## Test Cases
- Functional AC(`최소 6개 항목(연도/장소/한줄요약)`)를 충족하는 동작 확인
- 검증 경로: 수동 QA
