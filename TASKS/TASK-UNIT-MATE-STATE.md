...
## Context
동행글/요청 상태 전이 단위 테스트. 이 Task는 단위 테스트를 작성한다. 대상 범위: SCR-004. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 52에 대응한다.
...
## Depends On
API-MATES-WRITE, API-APPLICATIONS

## Expected Files
`src/lib/mates/state.test.ts`

이 목록 밖의 파일은 이 Task에서 수정하지 않는다.

## Functional AC
OPEN/CLOSED, PENDING/ACCEPTED/REJECTED 전이 규칙 검증
...
- ``src/lib/mates/state.test.ts`` 실행 시 100% 통과
