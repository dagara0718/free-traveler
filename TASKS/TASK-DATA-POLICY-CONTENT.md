# DATA-POLICY-CONTENT — 이용약관·개인정보·안전수칙·면책 정적 콘텐츠
...
## Context
이용약관·개인정보·안전수칙·면책 정적 콘텐츠. 이 Task는 `src/data`의 정적 데이터를 작성한다. DB 접근 코드를 포함하지 않는다. 대상 범위: SCR-003, SCR-005. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 49에 대응한다.
...
## Test Cases
- Functional AC(`정책 버전 필드 포함(동의 시각 기록용)`)를 충족하는 동작 확인
- 검증 경로: 수동 QA
