# API-APPLICATIONS — 참가요청 생성/승인/거절
...
## Context
참가요청 생성/승인/거절. 이 Task는 Supabase 기반 서버 로직(Server Action/Route Handler)을 구현한다. 대상 범위: SCR-004, SCR-005. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 36에 대응한다.

## Project Scope
Implementation Status: **IMPLEMENT** (`docs/PROJECT_SCOPE.md`, `docs/UIUX_TRACEABILITY.md` 기준).

## Requirement Ref
REQ-FUNC-034,REQ-FUNC-035,REQ-FUNC-036,REQ-FUNC-043,REQ-FUNC-044
...
## Expected Files
`src/app/api/mates/[id]/applications/route.ts`, `src/app/api/applications/[id]/route.ts`

이 목록 밖의 파일은 이 Task에서 수정하지 않는다.

## Functional AC
비작성자 상태변경 403; 인앱 알림 상태 1분 이내 생성
...
## Test Cases
- Functional AC(`비작성자 상태변경 403; 인앱 알림 상태 1분 이내 생성`)를 충족하는 동작 확인
- 검증 경로: UNIT-MATE-STATE, TEST-RLS-BASIC
...
- [ ] Verify(UNIT-MATE-STATE, TEST-RLS-BASIC) 통과
