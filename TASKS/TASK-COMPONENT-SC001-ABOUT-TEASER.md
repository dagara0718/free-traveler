# COMPONENT-SC001-ABOUT-TEASER — 대표 소개 요약 카드
...
## Context
대표 소개 요약 카드. 이 Task는 하나 이상의 Page Owner Task가 의존하는 화면 구성요소를 구현한다. 대상 범위: SCR-001. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 15에 대응한다.
...
## Functional AC
`50+ Trips`/`30+ Countries` 홈·About 값 일치
...
## Test Cases
- Functional AC(``50+ Trips`/`30+ Countries` 홈·About 값 일치`)를 충족하는 동작 확인
- 검증 경로: 수동 QA
