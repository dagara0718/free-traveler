...
## Context
차단 생성/해제. 이 Task는 Supabase 기반 서버 로직(Server Action/Route Handler)을 구현한다. 대상 범위: SCR-004, SCR-005. 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 37에 대응한다.

## Project Scope
Implementation Status: **IMPLEMENT** (`docs/PROJECT_SCOPE.md`, `docs/UIUX_TRACEABILITY.md` 기준).

## Requirement Ref
REQ-FUNC-040,REQ-FUNC-044
...
## Test Cases
- Functional AC(`차단 즉시 상호 콘텐츠 비노출 반영`)를 충족하는 동작 확인
